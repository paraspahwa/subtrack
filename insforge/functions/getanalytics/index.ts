...
export default async function (req: Request): Promise<Response> {
  ...
  const monthlyTotal = (subs || []).reduce((acc: number, s: any) => acc + convertedMonthlyCost(s, homeCurrency), 0);
  ...
  const wasteSubs = (subs || []).filter((s: any) => s.usage_rating && s.usage_rating <= 2);
  const wasteMonthly = wasteSubs.reduce((acc: number, s: any) => acc + convertedMonthlyCost(s, homeCurrency), 0);
